<template>
  <div :style="{ 'height': 'calc(' + store.getters.HomeHeight + ')' }" id="pro">
    <div class="pro scroll pro2" @wheel="test($event, 'PC')" @touchend="test($event, 'PE')" @touchstart="test($event, 'PE')"
      @touchmove="AllowMove">
      ...
      <div v-for="(item, index) in ProList" :key="'pro' + index" class="ProBox" @click="ChosePro(item)">
        <div v-lazy="item.img"
          style="width: 100%;margin-bottom: 20px;border-radius: 5px;overflow: hidden;text-align: center;">
          <img src="" alt="加载中" srcset="" style="max-width: 100%;" class="ProImg">
        </div>
        <h3>{{ item.title }}</h3>
...
const ProList = reactive([{
  ...
  img: new URL('@/assets/img/repeatsm.jpg', import.meta.url).href, title: '更多❓', info: '还想查看其他项目？', tag: '❓', tags: ['Vue', 'Uniapp', 'Less'], detail: { info: '其实我还有大屏与地图项目还有一些其他的web与移动端，但是因为已经很久了，所以没有截图，想了想还是不放出来了。想了解更多项目就问我本人吧。', imgs: [new URL('@/assets/img/repeatsm.jpg', import.meta.url).href] }
}])
